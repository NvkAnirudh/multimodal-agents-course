...
from recollect_api.agent.memory import Memory
...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.tool_response_system_prompt = None
    ...
    async def setup(self):
        ...
        async with self.mcp_client as _:
            ...
            self.tool_response_system_prompt = await self._get_tool_response_system_prompt()
    ...
    async def _get_tool_response_system_prompt(self) -> str:
        """Get the tool response system prompt."""
        logger.info("Getting tool response system prompt")
        mcp_prompt = await self.mcp_client.get_prompt("tool_response_system_prompt")
        return mcp_prompt.messages[0].content.text
