<p align="center">
    ...
    <h1 align="center">🔌 Recollect MCP 🔌</h1>
    <h3 align="center">An MCP server for serious video processing</h3>
</p>

Welcome to the Recollect MCP server!

This is the brains behind the scenes: generating prompts, managing resources, and handling the tools Recollect needs to process your videos.

Recollect MCP is one of three core components of the full Recollect app (check out the [Docker Compose](../docker-compose.yml) file for the full stack), but it can also run on its own if needed.
...
Recollect MCP is a Python project at heart, so getting it up and running locally is straightforward.
...
The `OPENAI_API_KEY` is used for image captioning and embedding. The rest are for Opik, our tool for managing and versioning Recollect prompts (which live in the MCP server and are accessed by the API).
